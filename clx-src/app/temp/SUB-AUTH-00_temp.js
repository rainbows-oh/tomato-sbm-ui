 ...
 *******************************************************************************/
 exports.openLoadPage = doLoadContent;
 ...
 *******************************************************************************/
 function onBodyLoad(e) {
	// 1. AppStack 이벤트 등록
	util.AppStack.start(function(vsAppId) {
		if (!vsAppId) return;
		var cleanAppId = vsAppId.replace(/\$\d+$/, "");
		// 캡처한 mainAppInstance를 통해 임베디드앱 조회
		var vcEmb = app.lookup("eaContent");
		util.EmbApp.setPage(app, "eaContent", cleanAppId);
	}, false);
}
