 ...
 *   } poProp? 팝업 설정 속성
 */
DialogKit.prototype.open = function(app, psAppId, pnWidth, pnHeight, pfHandler, poInitValue, poProp) {
	if (poInitValue == null) {
		poInitValue = {};
	}
	...
	var vsDialogId = app.id + "_" + psAppId;
	...
	voDialogMngr.openDialog(psAppId, vsDialogName, voDialogProp, function( /* cpr.controls.Dialog */ dialog) {
 ...
 * @param {Boolean} pbFocus? 그리드 포커스 여부
 * @return {void}
 */
GridKit.prototype.selectRow = function(app, psGridId, pnRowIndex, pbEmitEvent, pbFocus) {
	/** @type cpr.controls.Grid */
	var vcGrid = app.lookup(psGridId);
	if (pnRowIndex == null || pnRowIndex == undefined) {
		pnRowIndex = this.getIndex(app, psGridId);
	}
	var vbFocus = ValueUtil.isNull(pbFocus) ? true : pbFocus;
...
GridKit.prototype.exportMassiveData = function(app, psGridId, psFileName, poOptions) {
	...
	//엑셀 익스포트시 suppress, mergedToIndexExpr에 의한 셀 병합을 반영할지 여부(false)
	poOptions = poOptions || {};
	...
	function sendDataToWorker(pnStart) {
		...
		vcSubExport.action = window.location.origin + "/massive/excelExport.do";
...
/**
 * 그리드에 보여지는 대용량 데이터를 PDF export 하는 API (서버 분할 전송 방식)
 * 제약사항
 * 1. 그룹, 그룹헤더는 제공하지 않습니다.
 * 2. 병합은 제공하지 않습니다.
 * @param {cpr.core.AppInstance} app app인스턴스
 * @param {#grid} psGridId export할 그리드 Id
 * @param {String} psFileName 다운로드할 PDF파일 이름
 * @param {{
 *   excludeColumns? : String <!-- 출력시 제외할 컬럼명 (여러개인 경우 콤마로 구분) ex) COL1,COL2,COL3 -->,
 *   isExcludeHideColumn? : Boolean =`true` <!-- 숨김컬럼 제외 여부 -->,
 *   rows?: Number[] <!-- 익스포트시킬 특정 row index 배열 -->,
 *   showRowIndex? : Boolean =`false` <!-- rowindex 특수컬럼을 export 할지 여부 -->,
 *   showLoadmask? : Boolean =`true` <!-- 로드마스크 표시 여부 -->
 * }} poOptions? 그리드 PDF 익스포트 옵션
 */
GridKit.prototype.exportMassivePdfData = function(app, psGridId, psFileName, poOptions) {
	var _this = this;
	/** @type cpr.controls.Grid */
	var vcGrid = app.lookup(psGridId);
	...
	// 기본 출력 제외 컬럼(인덱스 컬럼, 선택용 체크 컬럼 등)
	var voColumn;
	var vaExcludeCollIndices = [];
	...
	if (!ValueUtil.isNull(poOptions.excludeColumns)) {
		...
		for (var j = 0, jlen = vaExclColumns.length; j < jlen; j++) {
			vaDColumns = vcGrid.detail.getColumnByName(vaExclColumns[j].trim());
			if (vaDColumns) {
				vaDColumns.forEach(function(each) {
					vaExcludeCollIndices.push(each.colIndex);
				});
			}
		}
	...
	// 그리드 전체 메타데이터 추출 (헤더 정보용)
	var voExportData = vcGrid.getExportData(voExportOption);

	// Detail 데이터 영역은 Chunk 전송을 위해 초기 세팅에서 제거
	if (voExportData.rowgroups.length > 1) {
		voExportData.rowgroups.splice(1, 1);
	}
	...
	// =========================================================================
	// [보완 1] 조회된 데이터가 0건일 때 무의미한 전송 방지 및 알림 처리
	// =========================================================================
	if (vnTotalRows === 0) {
		_this._appKit.Msg.alertDlg(app, "출력할 데이터가 없습니다."); // 해당 공통 메시지 코드 사용 가능
		return;
	}

	var vnDataSize = 2000; // Chunk 분할 단위 (2,000건씩 분할 전송)
	...
	function sendDataToWorker(pnStart) {
		...
		var vnCurrentProgress = Math.min(pnStart + vnAdjustedSize, vnTotalRows);
		if (voActiveLoadmask) {
			voActiveLoadmask.module.count(vnCurrentProgress);
			voActiveLoadmask.module.progress(vnTotalRows, vnCurrentProgress);
		}

		voExportOption.rows = vaRowIndex;
		var voData = vcGrid.getExportData(voExportOption);
		var vbIsLastSend = (pnStart + vnDataSize >= vnTotalRows) || (vnTotalRows == 0);
		var vbIsFirstSend = (pnStart == 0);
		...
		var voSendObj = {
			...
			isLastSend: vbIsLastSend,
			isFirstSend: vbIsFirstSend,
			data: voData.rowgroups ? voData.rowgroups[1] : null,
			gridInfo: vbIsFirstSend ? voExportData : null, // 첫 번째 전송일 때만 gridInfo 전송
			pdfKey: vsPdfKey // 이전 요청 성공 시 업데이트된 pdfKey를 전달
		};

		if (vbIsLastSend && voFooter != null) {
			voSendObj.footer = voFooter;
		}

		// =========================================================================
		// 1. 마지막 Chunk 전송: fetch API 사용 (파일 다운로드 팝업 처리)
		// =========================================================================
		if (vbIsLastSend) {
			fetch(window.location.origin + "/massive/pdfExport.do", {
				method: "POST",
				headers: {
					"Content-Type": "application/json;massdata"
				},
				body: JSON.stringify({ reqObj: voSendObj })
			})
			...
			.then(function(blob) {
				...
				a.href = url;
				a.download = (psFileName || "export_data") + ".pdf";
		...
		// =========================================================================
		// 2. 중간 Chunk 데이터 전송 (eXBuilder6 Submission 사용)
		// =========================================================================
		/** @type cpr.protocols.Submission */
		var vcSubExport = new cpr.protocols.Submission();
		vcSubExport.action = window.location.origin + "/massive/pdfExport.do";
		vcSubExport.mediaType = "application/json;massdata";
		vcSubExport.setRequestObject(voSendObj);

		vcSubExport.addEventListener("submit-success", function(e) {
			var sub = e.control;
			var vsXhrText = sub.xhr.responseText;
			var vbHasPdfKey = false;
			
			try {
				var voJsonObj = JSON.parse(vsXhrText);
				// [보완 2] 서버가 반환한 pdfKey 세팅 및 유효성 확인
				if (voJsonObj && !ValueUtil.isNull(voJsonObj["pdfKey"])) {
					vsPdfKey = voJsonObj["pdfKey"];
					vbHasPdfKey = true;
				}
			} catch (err) {}
			...
			// [보완 2-1] 첫 전송이 아닌데 pdfKey를 얻지 못한 경우 무한 루프/에러 전송 방지
			if (!vbHasPdfKey && !vbIsFirstSend) {
				if (poOptions.showLoadmask) {
					_this._appKit.hideLoadMask(app);
				}
				_this._appKit.Msg.alertDlg(app, "ERR-M011");
			...
			var vnNextStart = pnStart + vnDataSize;
			sendDataToWorker(vnNextStart);
		});
...
GridKit.prototype.exportData = function(app, psGridId, psFileName, poMetadata, poOptions, poCallback) {
	...
	var vcSubExport = new cpr.protocols.Submission();
	vcSubExport.action = window.location.origin + "/export/" + psFileName.replace("\/", "") + "." + poOptions.fileType;
...
/**
 * 반응형 그리드가 적용되어 있는 경우,<br>
 * screen-change 시 반응형 동작이 완료 된 후, 추가 콜백 함수를 정의합니다.<br>
 * 콜백함수는 onBodyLoad 에서 최초 1회만 작성하면 됩니다.<br>
 * 
 * <pre><code>
 * (예시)
 * function onBodyLoad() {
 * 	util.Grid.callRGridScreenChange(app, "grd1", function(e) {});
 * } 
 * </code></pre>
 * @param {cpr.core.AppInstance} app 앱인스턴스
 * @param {#grid} psGridid 반응형 그리드ID
 * @param {(evt: cpr.events.CScreenChangeEvent)=>void} pfCallback screen-change 시 반응형 동작 후 동작할 콜백 함수
 */
GridKit.prototype.callRGridScreenChange = function (app, psGridid, pfCallback) {
	/** @type cpr.controls.Grid */
	var vcGrid = app.lookup(psGridid);
	if(!vcGrid) return;
	...
	// screen-change 이벤트 콜백 핸들러 추가
	voRGrid._callbackScreenChangeEvent = pfCallback;
	
	// 화면 로드 시, callback 함수 동작하도록 최초 1회 호출
	pfCallback(new cpr.events.CScreenChangeEvent({name: app.targetScreen.name, media : app.targetScreen.media}));
}
...
SubmissionKit.prototype._onSubmitReceive = function(e, pbSuccess) {
	...
	// =========================================================================
	// [수정] 응답 데이터가 body 객체로 감싸져 내려올 경우 
	// body 내부의 dataset / datamap target을 찾아 데이터를 직접 build
	// =========================================================================
	
	if (voJsonRes && voJsonRes.body) {
		var voModel = voJsonRes.body
		for (var key in voModel) {
			var vcDs = _app.lookup(key);
			if (vcDs && typeof vcDs.build === "function") {
				vcDs.build(voModel[key]);
			}
		}
	}
	
	var voErrMsgInfo = voJsonRes["success"];
	if (ValueUtil.fixNull(voErrMsgInfo) == "false") {
		var vsErrMsg = "";
		try {
			var vsErrMsg = "\"" + voJsonRes["message"] + "\"";
			vsErrMsg = Function('"use strict";return (' + vsErrMsg + ')')();
		} catch (e) {
			vsErrMsg = voJsonRes["message"];
		}
...
SubmissionKit.prototype.send = function(app, psSvcId, successCallback, pbAppEnable, maskType) {
	...
	if (vcSubmission.action.indexOf("/") == 0) {
		vcSubmission.action = window.location.origin + vcSubmission.action;
	}
