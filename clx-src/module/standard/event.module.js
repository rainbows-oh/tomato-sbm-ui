...
for(var i = 1900; i < 3000; i++) {
	var holidayList = DateUtil.getHoliday(i);
	for(var j = 0; j < holidayList.length; j++) {
		
		cpr.controls.Calendar.addAnniversary({date: holidayList[j], className: "holiday"}, "YYYYMMDD");
	}
}
		...
		if (each instanceof cpr.controls.Container) {
			...
			if (each.getLayout() instanceof cpr.controls.layouts.FormLayout && !ValueUtil.isNull(each.userAttr("mobile-column-count")) || !ValueUtil.isNull(each.userAttr("tablet-column-count"))) {
				if (ValueUtil.isNull(each["_RForm"])) {
					...
					var vaMobileScrnName = voForm._screenNms.mobile.filter(function(scrn) {
						return (vaSupportedMediaName.indexOf(scrn) != -1);
					});
					if (vaMobileScrnName.length > 0) voForm.setColumnSettings(vaMobileScrnName[0], parseInt(each.userAttr(voForm.ATTR_NM.ATTR_MOBILE_COLUMN_COUNT) || "0"));
					
					// responsiveForm - tablet col settings
					var vaTabletScrnName = voForm._screenNms.tablet.filter(function(scrn) {
						return (vaSupportedMediaName.indexOf(scrn) != -1);
					});
					if (vaTabletScrnName.length > 0) voForm.setColumnSettings(vaTabletScrnName[0], parseInt(each.userAttr(voForm.ATTR_NM.ATTR_TABLET_COLUMN_COUNT) || "0"));
	...
	if(AppProperties.RUN_APP_INIT_TASK === true) {
		cpr.events.EventBus.INSTANCE.addFilter("init", function(e) {
			if (e.control instanceof cpr.core.AppInstance) {
				...
				var _app = e.control; // 앱 인스턴스
				var vaCtl = _app.getContainer().getAllRecursiveChildren(true); // 대상 컨트롤
